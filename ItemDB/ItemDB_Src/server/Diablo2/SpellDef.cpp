...
DWORD D2GetClassName(int nClassID, LPTSTR lpszBuffer, DWORD dwMaxChars)
{
	...
	{
	case CLASS_AMA:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("ÑÇÂíÑ·") : _T("Amazon"), dwMaxChars);
		break;

	case CLASS_SOR:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("Å®Î×") : _T("Sorceress"), dwMaxChars);
		break;

	case CLASS_NEC:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("ÄÐÎ×") : _T("Necromancer"), dwMaxChars);
		break;

	case CLASS_PAL:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("Ê¥ÆïÊ¿") : _T("Paladin"), dwMaxChars);
		break;

	case CLASS_BAR:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("Ò°ÂùÈË") : _T("Barbarian"), dwMaxChars);
		break;

	case CLASS_DRU:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("µÂÂ³ÒÁ") : _T("Druid"), dwMaxChars);
		break;

	case CLASS_ASN:
		_tcsncpy(lpszBuffer, g_bLanguage ? _T("´Ì¿Í") : _T("Assassin"), dwMaxChars);
		break;
