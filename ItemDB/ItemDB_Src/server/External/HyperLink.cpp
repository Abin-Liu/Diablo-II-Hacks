...
CHyperLink::CHyperLink()
{
    ...
    m_crVisitedColour   = RGB( 128,  0, 128);   // Purple
    m_crHoverColour     = RGB(255,   0,   0);   // Red
	m_crBgColour		= ::GetSysColor(COLOR_3DFACE); // default background colour
...
BOOL CHyperLink::OnEraseBkgnd(CDC* pDC) 
{
    CRect rect;
    GetClientRect(rect);
    pDC->FillSolidRect(rect, m_crBgColour);
...
void CHyperLink::SetColours(COLORREF crLinkColour, COLORREF crVisitedColour,
							...
							COLORREF crBgColour /* = -1 */) 
{ 
    m_crLinkColour    = crLinkColour; 
    m_crVisitedColour = crVisitedColour;

	if (crHoverColour == -1)
		m_crHoverColour = RGB(255, 0, 0); //::GetSysColor(COLOR_HIGHLIGHT);
	else
		m_crHoverColour = crHoverColour;

	if (crBgColour == -1)
		m_crBgColour = ::GetSysColor(COLOR_3DFACE);
	else
		m_crBgColour = crBgColour;
...
void CHyperLink::SetUnderline(int nUnderline /*=ulAlways*/)
{
