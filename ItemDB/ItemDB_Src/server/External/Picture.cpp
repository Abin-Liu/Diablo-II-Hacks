...
//         (Updated by Abin, 2003-8-16, wether to stretch is now determined
//         by the "bStretch" parameter. TRUE=stretch, FALSE=Not Stretch)
//
...
// ~~~~~~
//-----------------------------------------------------------------------------
BOOL CPicture::Draw(CDC *pDC, CRect DrawRect, BOOL bStretch)
//=============================================================================
{
	if (!bStretch)
		return Draw(pDC, DrawRect.TopLeft());
...
// ~~~~~~
//-----------------------------------------------------------------------------
BOOL CPicture::Draw(CDC *pDC, CPoint LeftTop, CPoint WidthHeight, int MagnifyX, int MagnifyY)
//=============================================================================
{
    if (pDC == NULL || m_IPicture == NULL) return FALSE;
...
//         does not want the images to be Streached and does not know the original
//         image dimensions.
BOOL CPicture::Draw(CDC *pDC, POINT LeftTop)
{
	return Draw(pDC, LeftTop, CPoint(m_Width, m_Height), 0, 0);
}

BOOL CPicture::Load(LPBYTE lpData, DWORD dwDataSize)
{
	if (!LoadPictureData(lpData, dwDataSize))
		return FALSE;

	m_Weight = dwDataSize; // Update Picture Size Info...
...
SIZE CPicture::GetPictureSize() const
{
	SIZE cz = { 0 };
	if (IsValid())
	{
...
{
	return m_IPicture && m_Height && m_Width;
}
