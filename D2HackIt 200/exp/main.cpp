...
#include "..\\Includes\\Utilities\\StringEx.h"	  // Include the backend of the module
...
BOOL PRIVATE OnGameCommandCodes(char** argv, int argc);
...
MODULECOMMANDSTRUCT ModuleCommands[]=
{
	...
	//	"codes",
	//	OnGameCommandCodes,
	//	"codesÿc0 Specifies codes of types."
	//},
...
BOOL PRIVATE OnGameCommandCodes(char** argv, int argc)
{
	...
	server->GameInfof("Codes: ÿc9%s", (LPCSTR)g_sCodes);
	return TRUE;
}
...
{	
	// Put debug code here...
	DWORD dwItemID = me->GetStorageItem(STORAGE_INVENTORY, 0, 0);

	if (dwItemID == 0)
	{
		server->GameErrorf("No item at inventory location 0,0");
		return TRUE;
	}

	if (argc < 3)
	{
		server->GameErrorf("Syntax: .exp parse <expression>");
		return TRUE;
	}
	...
	char szName[256] = "";
	server->GetItemDescription(dwItemID, szName, 255, TRUE);

	//server->GameInfof("Codes: ÿc9%s", (LPCSTR)g_sCodes);
	server->GameInfof("Item: %s", szName);
	server->GameInfof("Expression: ÿc9%s", (LPCSTR)sExp);

	BOOL bError = FALSE;
	BOOL bRes = server->ExamItemProperties(dwItemID, sExp, g_sCodes, &bError);
	
	if (!bError)
		server->GameInfof("Result: ÿc2%d", bRes);
...
{
	//if (nMessage == UM_ITEMEVENT && wParam == ITEM_EVENT_TO_STORAGE)
	//	server->GameInfof("type %d", server->GetItemType(lpUnit->dwUnitID));
	//if (nMessage == UM_ITEMEVENT)
	//{
	//	server->GameInfof("drop");
	//	SIZE sz = server->GetItemSize2(lpUnit->dwUnitID);
	//	server->GameInfof("x %d, y %d", sz.cx, sz.cy);
	//}
}
