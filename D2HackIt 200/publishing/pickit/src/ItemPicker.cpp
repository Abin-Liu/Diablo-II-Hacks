...
BOOL CItemPicker::ProcessItemQueue(ItemList &queue)
{
	...
	while (pos)
	{
		...
		gu.dwUnitID = si.dwItemID;
		DWORD dwMode = server->GetUnitMode(&gu);
		if (dwMode != ITEM_MODE_ON_GROUND && dwMode != ITEM_MODE_BEING_DROPPED)
		{
			// not a ground item
			//server->GameInfof("not ground item");
			queue.RemoveAt(cur);
			continue;
		}
		...
		{
			//server->GameInfof("item too far %04X %04X", si.x, si.y);
		...
		si.nRetry++;
		if (si.nRetry * (int)m_dwDelay > 1500)
		{
			si.nRetry = 0;
