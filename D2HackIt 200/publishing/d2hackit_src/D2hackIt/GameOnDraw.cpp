...
int D2GetBkMode();
POINT D2GetCurrentPosition();
BOOL D2DrawD2Bitmap(HD2BITMAP hBitmap, long x, long y, BYTE iMaskColor = -1);
...
void InitializeGameDC()
{
	...
	BIND_TO_GAMEDC(GetCurrentPosition);
	BIND_TO_GAMEDC(DrawD2Bitmap);
...
BOOL D2DrawD2Bitmap(HD2BITMAP hBitmap, long x, long y, BYTE iMaskColor)
{
	if (!CheckDummyError("D2DrawD2Bitmap"))
		return FALSE;

	return DrawBmpToScreen(hBitmap, x, y, iMaskColor);
}
