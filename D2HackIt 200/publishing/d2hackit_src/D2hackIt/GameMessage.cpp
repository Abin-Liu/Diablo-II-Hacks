...
void ServerPacketAfterReceived(const BYTE* aPacket, DWORD aLen)
{
	...
	*/
	...
	//////////////////////////////////////////////////////////
	// Inventory Gold Update
	//////////////////////////////////////////////////////////
	if ((aPacket[0] == 0x1d || aPacket[0] == 0x1e || aPacket[0] == 0x1f)
		&& (aPacket[1] == 0x0e || aPacket[1] == 0x0f))
	{
		DWORD dwStashGold = g_dwStashGold;
		DWORD dwInvGold = g_dwInvGold;
		g_dwStashGold = GetStat(STAT_GOLDBANK);
		g_dwInvGold = GetStat(STAT_GOLD);

		if (dwStashGold != g_dwStashGold)
		{
			SendThisPlayerMessage(PM_GOLDUPDATE, 1, g_dwStashGold);
		}

		if (dwInvGold != g_dwInvGold)
		{
			SendThisPlayerMessage(PM_GOLDUPDATE, 0, g_dwInvGold);
		}		
	}
}