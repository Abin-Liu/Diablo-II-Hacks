...
DWORD EXPORT CalculatePath(WORD x, WORD y, LPPATH lpBuffer)
{
...
MAPPOS EXPORT GetRandomPosition(int nRadiusMin, int nRadiusMax, int nAngleMin, int nAngleMax, LPCMAPPOS lpOrigin)
{	
	...
	if (lpOrigin == NULL)
		...
		::memcpy(&origin, lpOrigin, sizeof(MAPPOS));	
...
{	
	...
	POINT ptStart = { mp.x, mp.y };
	POINT ptEnd = { x, y };

	g_collisionMap.CreateMap();
	if (!g_collisionMap.IsValid())
		return 0;

	CWalkPath wp(&g_collisionMap);
	return wp.FindWalkPath(ptStart, ptEnd, lpBuffer);
}
