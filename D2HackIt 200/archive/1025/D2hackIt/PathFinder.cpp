...
{	
	...
	m_pCollisionMap->AbsToRelative(pos);
	...
	pos = best;
	Block(pos, nAdjust);	
	m_pCollisionMap->RelativeToAbs(pos);
...
DWORD CPathFinder::CalculatePathTo(WORD x, WORD y, LPPATH lpBuffer, int nAdjust)
{
	if (lpBuffer == NULL
		...
		|| !m_pCollisionMap->IsValidAbsLocation(x, y))
		return 0;
	
	memset(lpBuffer, 0, sizeof(PATH));
	lpBuffer->ptStart = GetPosition();
	if (lpBuffer->ptStart.x == 0 || lpBuffer->ptStart.y == 0)
		return 0;

	Reset();
	m_pCollisionMap->GetRechableBoundary(&m_rcBoundary, FALSE);
	CopyMapData();
	...
	m_pCollisionMap->AbsToRelative(m_ptRelDest);

	MakeMap2();	

	POINT pos;
	pos.x = (short)lpBuffer->ptStart.x;
	pos.y = (short)lpBuffer->ptStart.y;
...
BOOL CPathFinder::CopyMapData()
{
	...
	{
		for (int j = 0; j < MAP_LIMIT; j++)
			m_graph[i][j] = (m_pCollisionMap->GetMapData(i, j, FALSE) % 2) ? 0 : 1;
	}
