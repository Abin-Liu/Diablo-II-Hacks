...
BOOL g_bQuitOnVersionCheckDll = FALSE;
BOOL g_bQuitOnExtraworkDll = TRUE;
BOOL g_bIntegrityScanOnReload = FALSE;
...
BOOL __cdecl AntiDetectionSuspiciousDll(BYTE flag, LPCSTR lpDllFileName)
{
	...
	char szCmdLine[1024], szFileName[MAX_PATH];
	GetModuleFileNameA((HMODULE)g_hD2HackIt, szFileName, sizeof(szFileName));
	char *p = strrchr(szFileName, '\\');
	*p++ = '\0';
	wsprintf(szCmdLine, "\"%s\\d2reloader.exe\" %d %d \"%s\"", szFileName, GetCurrentProcessId(), g_bIntegrityScanOnReload, p);
	STARTUPINFO si;	
...
HMODULE __declspec(naked) __stdcall AntiDetectionVersionCheckingDllSTUB()
{
	__asm
	{
		...
		push g_bQuitOnVersionCheckDll; // keep me loaded, i.e. assuming version checking dlls are safe.
		jmp AntiDetectionSuspiciousDllSTUB;
	}
}
...
HMODULE __declspec(naked) __stdcall AntiDetectionExtraWorkDllSTUB()
{
	__asm
	{
		...
		nop;
		add dword ptr[esp], 4;
		push g_bQuitOnExtraworkDll; // set me free, i.e. assuming all extrawork dlls are NOT safe.
		jmp AntiDetectionSuspiciousDllSTUB;
	}
}
