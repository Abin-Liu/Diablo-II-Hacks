...
// Needed dlls force loading/unloading are moved to DllMain.cpp by Abin
...
//LPCSTR	NeededDlls[] = { "D2Common.dll", "D2Game.dll", "D2Client.dll", NULL };
...
BOOL PRIVATE ServerStart(HANDLE hModule)
{
	...
	::srand((unsigned int)::time(NULL)); // v2.00
...
//		return FALSE;
	...
	//////////////////////////////////////////////////////////////////
	si =		new SERVERINFO;					if(!si) goto memory_err;
	pfep =		new PRIVATEFUNCTIONENTRYPOINTS; if(!pfep) goto memory_err;
	thisgame =	new THISGAMESTRUCT;				if(!thisgame) goto memory_err;
	...
	if(GetIniString("D2HackIt", "Misc", "Verbose", tbf, sizeof(tbf)) && !stricmp(tbf, "on"))
		psi->Verbose = TRUE;
	else
		psi->Verbose = FALSE;
...
err_quit:
	...
	//if (!GetFingerprint("D2HackIt", "GamePacketReceivedIntercept3", psi->fps.GamePacketReceivedIntercept3)) 
	//{ 
	//	DGTRACE((DBGHDR "GamePacketReceivedIntercept3 NOT Found!\n"));
	//	goto err_quit;
	//}

	psi->fps.GamePacketReceivedIntercept3.AddressFound = 0x6FAB5374;
	psi->fps.GamePacketReceivedIntercept3.Offset = 0;
	psi->fps.GamePacketReceivedIntercept3.PatchSize = 6;
	...
	Intercept(INST_CALL,
		psi->fps.GamePacketReceivedIntercept3.AddressFound,
		(DWORD)GetFuncAddress(&GamePacketReceivedIntercept3STUB),
		psi->fps.GamePacketReceivedIntercept3.PatchSize);
	Intercept(INST_JMP,
	...
	g_bDrawBanner = !tbf[0] || stricmp(tbf, "0");
	
	tbf[0] = 0;
	GetIniString("D2HackIt", "Misc", "QuitOnVersionCheckDll", tbf, sizeof(tbf));
	if (tbf[0])
		g_bQuitOnVersionCheckDll = stricmp(tbf, "0");
	tbf[0] = 0;
	GetIniString("D2HackIt", "Misc", "QuitOnExtraworkDll", tbf, sizeof(tbf));
	if (tbf[0])
		g_bQuitOnExtraworkDll = stricmp(tbf, "0");
	tbf[0] = 0;
	GetIniString("D2HackIt", "Misc", "IntegrityScanOnReload", tbf, sizeof(tbf));
	if (tbf[0])
		g_bIntegrityScanOnReload = stricmp(tbf, "0");
