...
// PatchFill
// --------------------------------------------------------------------------
// Fills lpDest with lpSource, also save a copy of old code in lpBackup. dwCodeLen
// Applies to lpDest, lpSource, and lpBackup.
// Added 2.00 by Abin (abinn32@yahoo.com)
/////////////////////////////////////////////////////////////////////////////
BOOL EXPORT PatchFill(LPVOID lpDest, LPCVOID lpSource, LPVOID lpOldCode, int len)
{
	if (lpDest == NULL || lpSource == NULL || len <= 0)
		return FALSE;

	// Save old code first if lpOldCode isn't null
	BYTE* pBackup = NULL;
	if (lpOldCode)
	{
		pBackup = new BYTE [len];
		if (pBackup == NULL)
			return FALSE; // outta memory

		d2memcpy((DWORD)pBackup, (DWORD)lpDest, len);
	}	
	
	d2memcpy((DWORD)lpDest, (DWORD)lpSource, len);

	if (pBackup)
	{
		d2memcpy((DWORD)lpOldCode, (DWORD)pBackup, len);
		delete [] pBackup;
	}
