...
{
	if (aLen == 9 && aPacket[0] == 0x49 && aPacket[5] == 0x00)
	{
		if (!IsWpOpened())
...
DWORD ServerPacketBeforeReceived(BYTE* aPacket, DWORD aLen)
{
	...
	if ((aPacket[0] == 0x9c || aPacket[0] == 0x9d) && aPacket[1] != 0x13)
	{
		...
		QueueUnitMessage(UM_ITEMEVENT, &unit, aPacket[1], aPacket[3]);
	}
	...
	if (aPacket[0] == 0x95)
	{
		...
		{
			// hp up
			QueueThisPlayerMessage(PM_HPUP, wOldHP, g_wHP);
		}
		else if (wOldHP > g_wHP)
		{
			// hp down
			QueueThisPlayerMessage(PM_HPDOWN, wOldHP, g_wHP);
		}

		if (wOldMana < g_wMana)
		{
			//GameInfof("mana up");
			QueueThisPlayerMessage(PM_MANAUP, wOldMana, g_wMana);
		}
		else if (wOldMana > g_wMana)
		{
			//GameInfof("mana down");
			QueueThisPlayerMessage(PM_MANADOWN, wOldMana, g_wMana);
		}	
	}
