...
VOID EXPORT OnUnitMessage(UINT nMessage, LPCGAMEUNIT lpUnit, WPARAM wParam, LPARAM lParam)
{
	...
	if (nMessage == UM_HPPERCENT)
	{
		GAMEUNIT pet = { 0 };
		if (!me->GetMyMercUnit(&pet) || lpUnit->dwUnitID != pet.dwUnitID)
			return;		
...
DWORD EXPORT OnGameTimerTick(void)
{
	...
	//if ((server->GetInGameTime() % 1000) == 0)
	//	server->GameInfof("mana pending %d", g_bMyManaPending);
...
BOOL CheckAlert(int nAlertType, BYTE iValue)
{
	...
	if (nAlertType != ALERT_MERC && me->IsInTown())
	{
...
void DrinkPotion(BOOL bHealingOrMana, BOOL bMeOrMerc)
{
	...
	if (!bMeOrMerc)
	{
		if (GetPotionName(me->HealMerc(g_bKeepBottomBletRow), szPotion, 127))
		{
			server->GameInfof("Fed your merc a %s successfully.", szPotion);
			g_dwLastMercDrink = ::GetTickCount();
		}
		else
		{
	...
	else
	{
		if (bHealingOrMana)
		{
			if (GetPotionName(me->HealMe(g_bKeepBottomBletRow), szPotion, 127))
			{
				server->GameInfof("Drank a %s successfully.", szPotion);
				g_dwLastHealingDrink = ::GetTickCount();
			}
			else
			{
				server->GamePrintError("Unable to heal the player!");
			}
		}
		else
		{
			if (GetPotionName(me->SupplyMana(TRUE, g_bKeepBottomBletRow), szPotion, 127))
			{
				server->GameInfof("Drank a %s successfully.", szPotion);
				g_dwLastManaDrink = ::GetTickCount();
			}
			else
			{
				server->GamePrintError("Unable to supply mana!");
			}
		}
	}
}
