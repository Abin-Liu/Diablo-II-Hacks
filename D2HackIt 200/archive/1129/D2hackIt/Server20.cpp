...
BOOL CALLBACK AppEnumGroundItemProc(UnitPlayer* pPlayer, Room* pRoom, UnitAny* pUnit, LPARAM lParam1, LPARAM lParam2)
{
	...
	szCode[3] = '\0';
	return lpfn(pUnit->dwId, szCode, pUnit->hOPath->Xpos, pUnit->hOPath->Ypos, lParam2);
}
...
BYTE EXPORT GetUnitHPPercent(LPCGAMEUNIT lpUnit)
{
	...
	if (p == D2CLIENT_GetPlayerUnit())
		...
		return CalcPercent(D2COMMON_GetBaseStat(p, STAT_HP, 0) / 256, 128, 1);
}
...
BOOL EXPORT EnumPlayers(fnEnumPlayerProc lpfn, LPARAM lParam)
{
	...
	for (Roster* p = *p_D2CLIENT_FirstRosterUnit; p; p = p->ptNext)
	{
		...
		BYTE iMapID = (dwPvpFlags & PVP_ALLIED) ? p->LevelId : 0;
...
DWORD EXPORT GetItemDescription(DWORD dwItemID, LPSTR lpszBuffer, DWORD dwMaxChars, BOOL bUseColor)
{
	...
	if (bUseColor)
		p += GetItemColorHeader(dwItemID, szHeader);
	...
	::strncpy(lpszBuffer, szHeader, dwMaxChars);
	return ::strlen(lpszBuffer);
}
...
void DebugTest(char** argv, int argc)
{
	...
	g_collisionMap.ReportCollisionType(pt, 5);
	g_collisionMap.DumpMap("c:\\dump.txt", NULL, 0);
	*/

	/*
	SIZE cz = {1,1};
	POINT pt;
	FindFirstStorageSpace(STORAGE_CUBE, cz, &pt);
	GameInfof("best fit: %d, %d", pt.x, pt.y);
	//*/
}
