...
int CSyntax::Operate(int l, int r, char op)
{
	...
	case '/':
		...
		{
			__PRINTF("Error: Divided by zero.\n");
			res= 0;
		}
		break;
	case '+': 
...
int CSyntax::Parse(char *src)
{
	...
	while((p=GetAToken(src,token))!=NULL)
	{
		...
		case TOKEN_TYPE_OP:
			...
			{
				if(op_stack.empty()) return PARSING_FAIL;
				...
				if(num_stack.empty()) return PARSING_FAIL;
	...
	if(num_stack.size()>1)	return PARSING_FAIL;
