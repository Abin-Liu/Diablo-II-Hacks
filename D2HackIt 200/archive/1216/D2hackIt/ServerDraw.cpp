...
LPCSTR GetResistString(int nNum, LPSTR lpszBuffer);
LPCSTR GetStatString(int nBase, int nTotal, LPSTR lpszBuffer);
...
void DrawBanner(CGameDC* pDC, LPCRECT lpScreenRect)
{
	...
	pDC->SetFont(D2FONT_NORMAL);
	
	SIZE cText = pDC->GetTextExtent(szBanner);	
	pDC->TextOut(szBanner, lpScreenRect->right - cText.cx - 3, 0);
	/*
	RECT rect = {0};
...
void DrawMyStats(CGameDC* pDC, LPCRECT lpScreenRect)
{
	...
	rect.right = lpScreenRect->right - 3;
	rect.top = 3;
	rect.bottom = rect.top + 430;
	...
	// str
	sprintf(szText, "ÿc7Strength  %s", GetStatString(GetBaseStat(STAT_STRENGTH), GetStat(STAT_STRENGTH), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// dex
	sprintf(szText, "ÿc7Dexterity  %s", GetStatString(GetBaseStat(STAT_DEXTERITY), GetStat(STAT_DEXTERITY), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// vitality
	sprintf(szText, "ÿc7Vitality  %s", GetStatString(GetBaseStat(STAT_VITALITY), GetStat(STAT_VITALITY), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// energy
	sprintf(szText, "ÿc7Energy  %s", GetStatString(GetBaseStat(STAT_ENERGY), GetStat(STAT_ENERGY), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// life
	sprintf(szText, "ÿc7Max. Life  %s", GetStatString(GetBaseStat(STAT_MAXHP), GetStat(STAT_MAXHP), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// mana
	sprintf(szText, "ÿc7Max. Mana  %s", GetStatString(GetBaseStat(STAT_MAXMANA), GetStat(STAT_MAXMANA), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// file res
	num = GetStat(STAT_FIRERESIST);
	sprintf(szText, "ÿc7Fire Resist  %s  %s  %s",
		GetResistString(num, szTemp1), GetResistString(num - 40, szTemp2), GetResistString(num - 100, szTemp3));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// cold res
	num = GetStat(STAT_COLDRESIST);
	sprintf(szText, "ÿc7Cold Resist  %s  %s  %s",
		GetResistString(num, szTemp1), GetResistString(num - 40, szTemp2), GetResistString(num - 100, szTemp3));
	pDC->TextOut(szText, x, y);
	y += GAP;
	...
	sprintf(szText, "ÿc7Lightning Resist  %s  %s  %s",
		GetResistString(num, szTemp1), GetResistString(num - 40, szTemp2), GetResistString(num - 100, szTemp3));
	pDC->TextOut(szText, x, y);
	y += GAP;
	...
	sprintf(szText, "ÿc7Poison Resist  %s %s %s",
		GetResistString(num, szTemp1), GetResistString(num - 40, szTemp2), GetResistString(num - 100, szTemp3));
	pDC->TextOut(szText, x, y);
	y += GAP;

	// physic res
	sprintf(szText, "ÿc7Physic Resist  %s", GetResistString(GetStat(STAT_DAMAGERESIST), szTemp1));
	pDC->TextOut(szText, x, y);
	y += GAP;
	...
	GAMEUNIT gu = { 0 };
	GetMyMercUnit(&gu);
	sprintf(szText, "ÿc7Hiring Magic Findÿc0 %3d%%", GetUnitStat(&gu, STAT_MAGICFIND));
	pDC->TextOut(szText, x, y);
	y += GAP;	

	// draw "Press any key to close..."
	strcpy(szText, "ÿc8Press Any Key to Close");
...
{
	if (nNum >= 75)
		sprintf(lpszBuffer, "ÿc2%d", nNum);
	else if (nNum < 0)
		sprintf(lpszBuffer, "ÿc1%d", nNum);
	else
		sprintf(lpszBuffer, "ÿc0%d", nNum);
...
{
	if (nBase < nTotal)
		sprintf(lpszBuffer, "ÿc0%d  ÿc3%d", nBase, nTotal);
	else if (nBase > nTotal)
		sprintf(lpszBuffer, "ÿc0%d  ÿc1%d", nBase, nTotal);
	else
		sprintf(lpszBuffer, "ÿc0%d", nBase);
