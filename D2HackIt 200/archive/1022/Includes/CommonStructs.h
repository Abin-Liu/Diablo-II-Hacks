...
#define D2DRAW_FRAME		3	// Draw a rectangle frame on screen

#define D2DRAW_MAX_TEXT		255 // Maximum text length that can be drawn in one element
#define D2DRAW_ALIGN_LEFT			0	// Left align
#define D2DRAW_ALIGN_RIGHT			1	// Right align
#define D2DRAW_ALIGN_CENTER			2	// Center align(relative to screen width)
// Drawing Element Data Definition
typedef struct tagD2DrawData
{
	// Common stuff	
	int nDrawType;					  // D2DRAW_TEXT, D2DRAW_LINE, D2DRAW_RECT, D2DRAW_FRAME
	int nColor;						  // Color
	...
	int nAlign;						  // Alignment (Left, right, center)	
	int nFont;						  // Text font(0-13)

	// Stuff for line & rectangle/frame drawing
...
typedef BOOL (__cdecl *fnDrawScreen)(DWORD dwHookID, LPCD2DRAWDATA aData, int nDataCount);
...
typedef struct tagScreenEntryPoints
{
	...
	fnDrawScreen					DrawScreen;				// 2.00 (Abin)
