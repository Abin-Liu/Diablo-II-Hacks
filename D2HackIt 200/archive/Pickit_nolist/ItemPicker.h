...
public:		
	...
	BOOL RemoveFromQueue(ItemList& queue, DWORD dwItemID); // Removes a particular item from the queue
...
protected:
	static BOOL ExamConfigItem(LPCITEMCONFIG lpConfig, LPCGAMEUNIT lpUnit, LPCSTR* ppAnounce);
	static BOOL IsNeeded(DWORD dwItemID, LPCSTR lpszItemCode, const CItemPicker* pPicker, LPCSTR* ppAnounce);
	static BOOL CALLBACK EnumGroundItemProc(DWORD dwItemID, LPCSTR lpszItemCode, WORD x, WORD y, LPARAM lParam);
