...
LPWORD* AllocateMatrix(int cx, int cy)
{
	LPWORD* ppMatrix = NULL;
	if (cx <= 0 || cy <= 0)
		return NULL;
	
	ppMatrix = new LPWORD[cx];
	if (ppMatrix == NULL)
		return NULL;

	::memset(ppMatrix, 0, cx * sizeof(LPWORD));
	for (int i = 0; i < cx; i++)
	{
		ppMatrix[i] = new WORD[cy];
		if (ppMatrix[i] == NULL)
		{
			FreeMatrix(ppMatrix, cx, cy);
			return NULL;
		}
	}
...
void FreeMatrix(LPWORD*& ppMatrix, int cx, int cy)
{
	if (ppMatrix == NULL)
		return;
