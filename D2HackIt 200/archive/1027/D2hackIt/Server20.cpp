...
DWORD EXPORT CalculateTeleportPath(long x, long y, LPPOINT lpBuffer, DWORD dwMaxCount)
{
	...
	SIZE czMap = g_collisionMap.GetMapSize();
	POINT ptStart = GetPosition();
	POINT ptEnd = { x, y };
	g_collisionMap.AbsToRelative(ptStart);
	g_collisionMap.AbsToRelative(ptEnd);
	if (!IsValidIndex(ptStart, czMap) || !IsValidIndex(ptEnd, czMap))
		return 0;

	WORD** ppMap = AllocateMatrix(czMap.cx, czMap.cy);
	if (ppMap == NULL)
		return 0;

	g_collisionMap.CopyMapData(ppMap, czMap.cx, czMap.cy);
	g_collisionMap.MakeBlank(ppMap, czMap, ptStart);
	g_collisionMap.MakeBlank(ppMap, czMap, ptEnd);
	
	CTeleportPath tf(ppMap, czMap.cx, czMap.cy);
	DWORD dwCount = tf.FindTeleportPath(ptStart, ptEnd, lpBuffer, dwMaxCount);
	FreeMatrix(ppMap, czMap.cx, czMap.cy);
...
{	
	...
	CWalkPath wp(ppMap, czMap.cx, czMap.cy);
