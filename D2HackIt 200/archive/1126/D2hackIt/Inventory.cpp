...
int CStorageBuffer::GetAdjacent(int x, int y, int cx, int cy) const
{
	...
	for (v = y - 1; v <= y + cy; v++)
	{
		if (m_spaces.IsValidIndex(h, v) && m_spaces.GetAt(h, v) == 0)
			nCount++;
	}
...
BOOL CStorageBuffer::GetBestFit(int cx, int cy, LPPOINT lpBuffer) const
{
	...
	for (int i = 0; i < m_spaces.GetCX(); i++)
	{
		for (int j = 0; j < m_spaces.GetCY(); j++)
		{
			int nAdj = GetAdjacent(i, j, cx, cy);
			if (nAdj >= 0 && nAdj < nCurAdj)
			{
				lpBuffer->x = i;
				lpBuffer->y = j;
				nCurAdj = nAdj;
