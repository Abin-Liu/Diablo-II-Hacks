...
(		
	...
	"HideIt",				// Short module description
...
BOOL PRIVATE OnGameCommandToggle(char** argv, int argc)
{
	...
	if (g_bEnabled)
		server->GamePrintInfo("HideIt enabled");
	else
		server->GamePrintInfo("HideIt disabled");
	return TRUE;
}
...
{	
	...
	if (aPacket[1] != ITEM_ACTION_NEW_GROUND && aPacket[1] != ITEM_ACTION_DROP && aPacket[1] != ITEM_ACTION_OLD_GROUND)
		return aLen;
	...
	if (strcmp(item.szItemCode, "gld") == 0 && item.dwGoldAmount < 500)
		return 0;

	if (strcmp(item.szItemCode, "rvs") == 0 || strcmp(item.szItemCode, "rvl") == 0)
		return aLen;

	if (item.szItemCode[0] == 'r' && isdigit(item.szItemCode[1]) && isdigit(item.szItemCode[2]))
		return aLen;

	if (D2IsQuestItem(item.szItemCode))
		return aLen;

	if (item.iQuality < ITEM_LEVEL_MAGIC && !item.iEthereal && !item.iSocketNumber)
		return 0;
