...
{
	if (ServerPacketBeforeReceived(aPacket, aLength) == 0)
		return 0;
...
void __fastcall GamePacketReceivedIntercept3(BYTE* aPacket, DWORD aLength)
{
	ServerPacketAfterReceived(aPacket, aLength);
	...
	{	
		...
		{
			pCIS->OnGamePacketAfterReceived(aPacket, aLength);
		}
		li = ClientModules.GetNextItem(li);
	}	
}

void __declspec(naked) GamePacketReceivedIntercept3STUB()
{
	__asm {
		nop									// Make room for original code
		nop
		...
		test eax, eax
		jz done
		call eax // call packet handler
		mov edx, [esp+0x1c] // packet length
		mov ecx, ebp; // packet pointer

		// Call our clean c function
		call GamePacketReceivedIntercept3
done:
		ret
	}
}
