...
BOOL CSharedMem::InternalLock()
{
	// Lock the memory so other apps are not able to access
	return m_bExternalLocked ? TRUE : (::WaitForSingleObject(m_hMutex, m_dwTimeOut) == WAIT_OBJECT_0);
}

BOOL CSharedMem::InternalUnlock()
{
	// Unlock the memory
	if (m_bExternalLocked)
		return FALSE;
...
int CSharedMem::GetData(LPVOID pBuffer, DWORD dwSize, DWORD dwStartPosition)
{
	if (!m_bCreated)
	...
	InternalUnlock(); // Release mutex

	return nBlock; // Return the actual size(in bytes) copied
}

// Modify the memory contents
int CSharedMem::SetData(LPCVOID pData, DWORD dwSize, DWORD dwStartPosition)
{
	if (!m_bCreated)
		return ERR_NOTCREATED;
...
BOOL CSharedMem::Lock()
{
	...
	m_bExternalLocked = ::WaitForSingleObject(m_hMutex, m_dwTimeOut) == WAIT_OBJECT_0;
	return m_bExternalLocked;
}
