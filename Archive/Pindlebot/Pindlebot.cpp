...
BOOL CALLBACK OnHotkey(BYTE iVkCode, LPCTSTR lpszCommand)
{
	...
	{
		if (iVkCode == VK_F12 && g_bUseShopbot)
		{
			server->GamePrintInfo("Resuming pindlebot...");
			server->GameCommandLine("shopbot stop"); // hijack the "pindlebot resume" command if shopbot is running
		}
		else
		{
...
void EXPORT OnGameJoin(THISGAMESTRUCT* game)
{
	...
	server->GameCommandLine("shopbot autostart 0");
	...
	g_bUseShopbot = ini.GetBool("Default", "Use Shopbot", FALSE);	

	if (g_bUseShopbot)
	{
		// is shopbot.d2h there?
		sprintf(szConfig, "%s\\shopbot.d2h", GetModuleDirectory());
		g_bUseShopbot = ::GetFileAttributes(szConfig) != -1; 
	}	
	...
	if (g_bUseShopbot && g_dwIdleLength > 0)
	{
		sprintf(szConfig, "shopbot start %u", g_dwIdleLength / 1000);
		server->GameCommandLine(szConfig);
		server->GameCommandLine("shopbot callback pindlebot start");
		server->GameCommandLine("shopbot endact 5"); // back to act 5 after shopping is done
	}
	else
	{
...
BOOL PRIVATE OnGameCommandStart(char** argv, int argc)
{
	...
	DWORD dwWait = g_dwTotalTime < g_dwIdleLength ? g_dwIdleLength - g_dwTotalTime : 0;
	UpdateBotState(MS_IDLE, dwWait);
	return TRUE;
}
...
DWORD EXPORT OnGameTimerTick(void)
{
	...
	if (g_dwTotalTime > g_dwIdleLength && g_dwTotalTime - g_dwIdleLength > 1500000)
	{
		// stay for no more than 30 minutes, even if paused 
		FinishRun(FR_FAILED, FALSE);
	...
	case MS_IDLE:	
		UpdateBotState(MS_INIT_START, 2000);
		break;
...
DWORD EXPORT OnGamePacketBeforeReceived(BYTE* aPacket, DWORD aLen)
{
	...
	if (!Me.IsGameReady() && aPacket[0] == 0x9c && aPacket[1] == 0x12)
	{
...
BOOL PRIVATE OnGameCommandResume(char** argv, int argc)
{
	...
	g_bPaused = FALSE;
	Me.PauseJobs(FALSE);
	g_ticker.ResetTicker(max(g_ticker.GetRemainer(), 2000));
...
BOOL PRIVATE OnGameCommandDebug(char** argv, int argc)
{
	...
	D2Infof("Game Time: %u, Idle Length: %u, Game Ready: %d", g_dwTotalTime, g_dwIdleLength, Me.IsGameReady());
	return TRUE;
}
...
{
	if (!g_bUseShopbot && g_dwTotalTime < g_dwIdleLength)
	{
