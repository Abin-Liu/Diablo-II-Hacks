...
BOOL EXPORT OnClientStart()
{
	...
	g_picker.SetDelay(pickitConfig_ini.GetInt("Pickit", "Delay", PICKIT_DELAY_DEFAULT));
...
{
	g_picker.OnGameTimerTick(); // call every time tick
	return 0;
}

DWORD EXPORT OnGamePacketBeforeReceived(BYTE* aPacket, DWORD aLen)
{
	g_picker.OnGamePacketBeforeReceived(aPacket, aLen);
	return aLen;
}
...
{
	g_picker.SetDebug(!g_picker.IsDebug());
	//server->GameInfof("Valuables: %d, all: %d", g_picker.GetQueueItemCount(TRUE), g_picker.GetQueueItemCount(FALSE));
	g_picker.DebugQueue();
	return TRUE;
}
...
BOOL PRIVATE OnCommandDelay(char** argv, int argc)
{
	...
	pickitConfig_ini.WriteInt("Pickit", "Delay", g_picker.GetDelay());
	ShowDelaySetting();
	return TRUE;
}
