...
CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::CLinkedList()
{
	...
	char szMutex[256] = "";
	::sprintf(szMutex, "%s%08X", "{3FBB26D9-D352-409A-9352-7BDD78A1EBA1}", (DWORD)this);
	m_hMutex = ::CreateMutexA(NULL, FALSE, szMutex);
}
...
template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
BOOL CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::Lock(DWORD dwTimeout)
{
	return ::WaitForSingleObject(m_hMutex, dwTimeout) == WAIT_OBJECT_0;
}

template <class TYPE, class ARG_TYPE, void (CALLBACK *FREE_TYPE)(TYPE&)>
void CLinkedList<TYPE, ARG_TYPE, FREE_TYPE>::Unlock()
{
