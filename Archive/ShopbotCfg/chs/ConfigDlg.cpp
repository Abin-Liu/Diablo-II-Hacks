...
void CConfigDlg::OnOK() 
{
	...
	{
		MessageBox(_T("物品未设置任何属性！"), _T("无效数据"), MB_ICONEXCLAMATION);
		return;
	}

	if (m_sDesc.IsEmpty())
	{
		MessageBox(_T("\"文字说明\"不能为空！"), _T("无效数据"), MB_ICONEXCLAMATION);
		m_wndDesc.SetFocus();
	...
	{
		MessageBox(_T("\"Global\" 是保留词, 不能被用于文字说明！"), _T("无效数据"), MB_ICONEXCLAMATION);
		m_wndDesc.SetFocus();
		m_wndDesc.SetSel(0, -1);
		return;
	}
	...
	{
		CString sErr;
		sErr.Format(_T("\"%s\" 已经存在，请另选一个文字说明。"), m_sDesc);
		MessageBox(sErr, _T("无效数据"), MB_ICONEXCLAMATION);
		m_wndDesc.SetFocus();
		m_wndDesc.SetSel(0, -1);
		return;
	}
