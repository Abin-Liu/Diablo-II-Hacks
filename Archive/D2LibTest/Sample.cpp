...
void CALLBACK OnPlayerMessage(UINT nMessage, WPARAM wParam, LPARAM lParam, LPVOID lpData)
{
	...
	case PM_ACTCHANGE:
		D2Infof("Act changed from act %d to act %d", wParam + 1, lParam + 1);
		break;
...
VOID EXPORT OnGameJoin(THISGAMESTRUCT* game)
{
	...
	//draw.DrawSingleText("Hello whats a hell", 200, 200, 9);
	return;
}
...
BOOL PRIVATE OnCommandDebug(char **argv, int argc)
{
	...
	//Me.DebugMapData();
	//HMODULE h = ::LoadLibrary("D2Client");
	//D2Infof("h: %08X", (DWORD)h);
	draw.DrawSingleText("fuck this", 300, 300, 0);
