...
//////////////////////////////////////////////////////////////////////
#include "Classes\\D2Hackit\\D2Client.h"
...
char g_szModuleDirectory[MAX_PATH + 1] = ""; // The module directory, Abin
...
extern "C" BOOL APIENTRY DllMain(HANDLE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		// Get plugin path
		::strncpy(g_szModuleDirectory, szTemp, MAX_PATH);
		nLen = ::strlen(g_szModuleDirectory);
		while (nLen)
		{
			if (g_szModuleDirectory[nLen] == '\\')
			{
				g_szModuleDirectory[nLen] = '\0';
				nLen = 0;
			}
			else
			{
...
// Abin
LPCTSTR EXPORT GetModuleDirectory()
{
	return g_szModuleDirectory;
}
