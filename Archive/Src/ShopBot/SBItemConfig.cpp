...
#include <stdlib.h>
#include <stdio.h>
#include "..\\D2Lib\\\Classes\\Utils\\StringParse.h"
#include "..\\D2Lib\\\Classes\\Utils\\Ini.h"
...
BOOL ModMatch(LPCITEMMOD lpMod1, LPCITEMMOD lpMod2)
{
	...
	if (lpMod2->dwModID == 23) // +min damage
	{
		// it could be 21, 23, 159
		if (lpMod1->dwModID != 21 && lpMod1->dwModID != 23 && lpMod1->dwModID != 159)
			return FALSE;
	}
	else if (lpMod2->dwModID == 24) // +max damage
	{
		// it could be 22, 24, 160
		if (lpMod1->dwModID != 22 && lpMod1->dwModID != 24 && lpMod1->dwModID != 160)
			return FALSE;
	}
	else if (lpMod2->dwModID == 93) // ias
	{
		// it could be 68, 93
		if (lpMod1->dwModID != 68 && lpMod1->dwModID != 93)
			return FALSE;
	}
	else if (lpMod2->dwModID == 96) // fast run/walk
	{
		// it could be 67, 96
		if (lpMod1->dwModID != 67 && lpMod1->dwModID != 96)
			return FALSE;
	}
	else
	{
